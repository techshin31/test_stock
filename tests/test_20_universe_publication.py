...
def test_only_pass_runs_can_publish_after_same_day_deadline():
    config = load_config("risk_neutral")
    connection = PublishConnection(status="PASS")
    publish(
    ...
    with pytest.raises(UniversePublishError, match="only PASS"):
        publish(
            PublishDB(PublishConnection(status="WARNING")), 9, config,
            now_kst=datetime(2026, 6, 1, 8, 0),
        )
...
def test_new_warning_run_is_not_published(monkeypatch):
    from apps.worker.analyzer import pipeline
    from apps.worker.analyzer.models import AnalysisRunContext
    from apps.worker.analyzer.pipeline import build_request
