...
- `build_master_table.py`: 다개년 마스터 테이블과 섹터 벤치마크 생성
- `validate_master_table.py`: 다개년 검증 리포트 생성
- `build_sector_rankings.py`: 연도별·섹터별 기업 상대평가 생성
- `build_top_companies_report.py`: 다개년 상위 기업 요약 보고서 생성
- `prepare_text_targets.py`: 최신 연도 기준 텍스트 수집 대상 기업 선정
- `build_all_sector_analysis_report.py`: 2021~2025 종합 분석 보고서 생성
- `fetch_business_overview.py`: DART API를 이용한 사업개요 텍스트 수집
...
- `business_overview_text_latest.csv`
  - `fetch_business_overview.py`를 별도로 실행할 때 생성됩니다.
