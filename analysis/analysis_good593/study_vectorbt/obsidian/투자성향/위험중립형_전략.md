...
> 보유 종목(삼성전자·SK하이닉스·NAVER·현대차·KB금융)의 2022년 실제 MDD가 -40~-65%임을 감안한 현실적 목표.
...
| `use_adx_mode` | **전 종목 단일 값** ❌ | 종목별 dict |
| **자금 배분** (`build_size_df`) | **가중 평균** ❌ 신호 혼합 시 합계 불안정 | 3단계 배분 로직 |
| 모멘텀 비례 배분 | 포트폴리오 전체 ✅ | 의도적 유지 |

```
개선 대상 코드:

backtest/portfolio.py — build_size_df()
  현재: size_raw × mom_weight  (신호 혼합 시 합계가 0.7 미만으로 떨어짐)
  목표: Case 1 — 신호 없음        → NaN,           ETF = 1.0
        Case 2 — 합계 ≤ 1.0      → size_raw 그대로, ETF = 나머지
        Case 3 — 합계 > 1.0      → size_raw×momentum 정규화 × 1.0, ETF = NaN

backtest/portfolio.py — _walk_forward_portfolio()
  현재: 그리드 서치 → 포트폴리오 전체 pf.value()로 IS score 1개 산출
  목표: 종목 루프 안으로 그리드 서치 이동 → 종목별 IS score·best_params·use_adx_mode 산출

profiles/neutral.py — get_signal()
