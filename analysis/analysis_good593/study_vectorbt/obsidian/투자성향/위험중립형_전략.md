...
sequenceDiagram
    ...
    WF->>PD: 직전 WF IS score (종목별 독립 산출)
...
sequenceDiagram
    ...
    loop 종목별 독립 실행
        ...
        OPT->>OPT: 평가 기준: 종목 단독 Calmar Ratio → 종목별 IS score 산출

        alt IS score > 0
            OPT->>PD: 해당 종목 best_params 갱신 (adx_threshold · adx_sideways)
            PD->>PD: OOS 3개월 — ADX 모드 적용
        else IS score ≤ 0
            Note over OPT,PD: 어떤 파라미터도 IS에서 수익 미달\nADX 파라미터 신뢰 불가
            OPT->>PD: 해당 종목 ADX 모드 비활성화
            PD->>PD: OOS 3개월 — MA+KOSPI 모드 적용\nKOSPI_MA60 필터로 하락장 UPTREND 차단 유지
        end
    end

    Note over PM: 모멘텀 비례 배분은 포트폴리오 수준 유지\n종목 간 모멘텀 비교로 비중 결정 — 종목별 분리 불가
...
평가 기준: 종목 단독 Calmar Ratio → 종목별 IS score 산출

[목표 설계 — 종목별 독립 평가]
  자동매매 주문은 종목별로 발생 → 판단 기준도 종목별이어야 함
  삼성전자(고변동성)와 한국전력(저변동성)에 동일 ADX 파라미터를 쓰면
  한쪽은 신호 과다, 다른 쪽은 신호 부재 발생

  종목별로 결정되어야 할 항목:
    best_params  (adx_threshold · adx_sideways)
    use_adx_mode (IS score > 0이면 True, 이하 False)

  → 한 종목의 IS score가 낮아도 다른 종목의 ADX 모드에 영향 없음

[포트폴리오 수준 유지 항목]
  모멘텀 비례 배분: 종목 간 모멘텀 비교로 비중을 결정 → 포트폴리오 단위에서만 계산 가능
  → 종목별 신호 생성(ADX 파라미터·모드) 완료 후 비중 결정을 포트폴리오 수준에서 수행

IS score > 0  → 해당 종목 best_params 적용 — ADX 모드
...
| 항목 | 현재 코드 | 목표 |
|------|----------|------|
| MA20>MA60>MA120 계산 | 종목별 ✅ | — |
| ADX 계산 | 종목별 ✅ | — |
| IS score (그리드 서치 평가) | **포트폴리오 전체** ❌ | 종목별 독립 산출 |
| `use_adx_mode` | **포트폴리오 전체 단일 값** ❌ | 종목별 dict |
| 모멘텀 비례 배분 | 포트폴리오 전체 ✅ | 의도적 유지 |

**개선 대상 코드 위치:**

```
backtest/portfolio.py
  _walk_forward_portfolio()
    현재: 그리드 서치 → 포트폴리오 전체 pf.value()로 IS score 1개 산출
           → use_adx_mode 1개로 전 종목 통일 적용
    목표: 그리드 서치를 종목 루프 안으로 이동
           → 종목별 IS score, best_params, use_adx_mode 산출
           → 비중 결정(build_size_df)만 포트폴리오 수준 유지

profiles/neutral.py
  get_signal()
    현재: use_adx_mode 단일 bool → 전 종목 동일 적용
    목표: use_adx_mode를 종목별 dict로 수신 → 종목마다 다르게 적용
```
