# 위험중립형 전략

> 관련: [[투자성향_분류]] | 확장: [[적극투자형_전략]]

---

## 성향 요약

"오를 때 충분히 벌고, 내릴 때 확실히 피한다"

- 하락장 회피(현금 대기) + 상승장 적극 매수
- MDD 최소화를 우선 목표로 설계
- 목표: 연 CAGR 6~10%, MDD -25% 이내

---

## 전략 실행 흐름 — 시퀀스 다이어그램

### 매일 장 실행 흐름

```mermaid
sequenceDiagram
    participant MKT  as 시장
    participant PD   as 국면 판별기
    participant ATR  as ATR 감시자
    participant SE   as 전략 실행기
    participant PM   as 포트폴리오 매니저
    participant ETF  as 단기채 ETF

    Note over MKT,ETF: 매일 장 시작

    MKT->>PD: 당일 OHLC + KOSPI 데이터

    Note over PD: 판별 우선순위: SIDEWAYS(횡보) → UPTREND(상승추세) → DOWNTREND(하락추세) → TRANSITION(전환)

    Note over PD: adx_sideways — Walk-Forward IS 12개월 학습, OOS 3개월 적용 [10·15·20]
    PD->>PD: ADX < adx_sideways?

    alt SIDEWAYS (횡보)
        PD->>ATR: ATR stop-loss 체크
        ATR->>ATR: 일일낙폭 > ATR × 2.0?
        alt ATR 발동
            ATR->>PM: 전량 청산 (0%)
            PM->>ETF: ETF 100% 자동 주차
        else 정상
            ATR->>SE: SIDEWAYS(횡보) 신호 전달
            SE->>SE: 볼린저밴드 하단 터치?
            SE->>PM: 매수 30%
            PM->>ETF: ETF 70%로 축소
            Note over SE: 볼린저밴드 상단 → 전량 청산
        end

    else ADX 기준 통과 → 추세 판별
        Note over PD: adx_threshold — Walk-Forward IS 12개월 학습, OOS 3개월 적용 [15·20·25·30]
        PD->>PD: MA20>MA60>MA120 AND ADX>threshold AND KOSPI>KOSPI_MA120?

        alt UPTREND (상승추세)
            PD->>ATR: ATR stop-loss 체크
            ATR->>ATR: 일일낙폭 > ATR × 2.0?
            alt ATR 발동
                ATR->>PM: 전량 청산 (0%)
                PM->>ETF: ETF 100% 자동 주차
            else 정상
                ATR->>SE: UPTREND(상승추세) 신호 전달
                SE->>SE: 비UPTREND → UPTREND 전환 첫날?
                SE->>PM: 1차 매수 40%
                PM->>ETF: ETF 60%로 축소
                Note over SE,PM: 60거래일 이내 종가 > MA20 지지 재확인 → 2차 매수 70%
            end

        else 역배열 확인
            PD->>PD: MA20<MA60<MA120 AND ADX>threshold OR KOSPI<KOSPI_MA120?

            alt DOWNTREND (하락추세)
                PD->>SE: DOWNTREND(하락추세) 신호
                Note over ATR: ATR 스킵 — 포지션 없음
                SE->>PM: 전량 청산 (0%)
                PM->>ETF: ETF 100% 자동 주차

            else TRANSITION (전환)
                PD->>ATR: ATR stop-loss 체크
                ATR->>ATR: 일일낙폭 > ATR × 2.0?
                alt ATR 발동
                    ATR->>PM: 전량 청산 (0%)
                    PM->>ETF: ETF 100% 자동 주차
                else 정상
                    ATR->>SE: TRANSITION(전환) 신호 전달
                    SE->>PM: 신규 진입 차단, 기존 포지션 유지
                end
            end
        end
    end

    PM->>PM: 모멘텀 비례 배분\n비중 × 종목모멘텀 / 유효모멘텀합
    PM->>ETF: ETF = 1 - 주식비중합\n(min_weight < 1% → NaN 유지)
```

### Walk-Forward 최적화 주기

```mermaid
sequenceDiagram
    participant CAL as 3개월 캘린더
    participant OPT as 최적화 엔진
    participant PD  as 국면 판별기

    Note over CAL,PD: 3개월마다 실행 (연 4회)

    CAL->>OPT: 최적화 트리거
    OPT->>OPT: IS 12개월 학습 구간 설정
    Note over OPT: IS 12개월 고정 이유 — MA120이 신뢰도 있게 계산되려면\n학습 구간이 MA120(6개월)보다 충분히 길어야 함
    OPT->>OPT: 그리드 탐색\nadx_threshold [15·20·25·30]\nadx_sideways  [10·15·20]\n→ 12가지 조합
    OPT->>OPT: 평가 기준: Calmar Ratio
    OPT->>PD: 최적 adx_threshold 갱신
    OPT->>PD: 최적 adx_sideways 갱신
    PD->>PD: OOS 3개월 적용 시작
    Note over PD: atr_multiplier=2.0 고정\nATR 값 자체가 매일 변하므로 최적화 불필요
```

---

## 매도 신호 우선순위 흐름

```mermaid
sequenceDiagram
    participant MKT as 시장
    participant SE  as 전략 실행기
    participant PM  as 포트폴리오 매니저
    participant ETF as 단기채 ETF

    Note over MKT,ETF: 보유 포지션 있음 — 우선순위 낮음 → 높음

    MKT->>SE: 국면 변화 감지

    SE->>SE: UPTREND(상승추세) → TRANSITION(전환) 전환?
    SE->>PM: 1차 익절 → 잔여 40% 유지
    PM->>ETF: ETF 60%로 증가

    SE->>SE: 데드크로스 (MA20 하향 돌파 MA60)?
    SE->>PM: 2차 청산 → 잔여 10% 유지
    PM->>ETF: ETF 90%로 증가

    SE->>SE: DOWNTREND(하락추세) 진입 or 볼린저밴드 상단?
    SE->>PM: 전량 청산 (0%)
    PM->>ETF: ETF 100% 주차

    Note over SE: ATR stop — 최우선, 위 신호 모두 덮어씀
    MKT->>SE: 당일 낙폭
    SE->>SE: 낙폭 > ATR × 2.0?
    SE->>PM: 즉시 전량 청산
    PM->>ETF: ETF 100% 긴급 주차
```

---

## 성과 평가 지표

### 절대 지표

> 보유 종목(삼성전자·SK하이닉스·NAVER·현대차·POSCO홀딩스)의 2022년 실제 MDD가 -40~-65%임을 감안한 현실적 목표.
> MA 후행성으로 인해 DOWNTREND 판별 시점은 이미 10~20% 하락 후인 경우가 많다.

| 구분 | 지표 | 목표 | 경보선 |
|------|------|------|-------|
| 수익성 | **CAGR** | 6~10% | < 4% |
| 손실 방어 | **MDD** | -25% 이내 | > -35% |
| 손실 지속 | **MDD Duration** | 12개월 이내 | > 18개월 |
| 효율 | **Calmar** | 0.3 이상 | < 0.2 |
| 하락 효율 | **Sortino** | 0.5 이상 | < 0.3 |

```
목표 도달 예시:
  CAGR 8%, MDD -25% → Calmar = 0.32  ✓
  CAGR 6%, MDD -30% → Calmar = 0.20  ⚠ 경보
  CAGR 5%, MDD -35% → Calmar = 0.14  ✗ 실패
```

### KOSPI 상대 지표

| 지표 | 목표 | 경보선 |
|------|------|-------|
| **Alpha** | ≥ 2%p | < 0 |
| **Beta** | < 0.8 | > 1.0 |
| **MDD 감소율** | > 20% | < 10% |
| **Calmar 개선** | > 0.1 | < 0 |
| **Information Ratio** | > 0.3 | < 0 |
| **승률** | > 50% | < 40% |

### 해석

```
판단 흐름:
  Alpha < 0                     → KOSPI 인덱스 펀드보다 못함 → 전략 실패
  Alpha ≥ 2%p AND Beta < 0.8    → 1차 통과 → 절대 지표 확인
  MDD > -35% OR Duration > 18M  → 경보 → 전략 재검토
  절대 지표 통과                 → Calmar/Sortino로 효율 확인

실전 보정:
  백테스팅 CAGR에서 1~2%p 차감이 현실적
  이유: 증권거래세(0.20%) + 실전 슬리피지(0.1~0.3%) > 백테스팅 설정(0.1%)
```

→ [[성과지표/CAGR]] · [[성과지표/MDD]] · [[성과지표/MDD_Duration]] · [[성과지표/Calmar비율]] · [[성과지표/Sortino비율]] · [[성과지표/샤프비율]]

---

## 각 전략의 내용 및 링크

### 시장 국면 판별

판별 우선순위: SIDEWAYS → UPTREND → DOWNTREND → TRANSITION

| 국면 | 조건 | 전략 요약 |
|------|------|---------|
| **SIDEWAYS** | ADX < adx_sideways | 볼린저밴드 하단 매수 30%, 상단 전량 청산 |
| **UPTREND** | MA20 > MA60 > MA120 AND ADX > adx_threshold AND KOSPI > KOSPI_MA120 | 전환 첫날 1차 매수 40%, MA20 지지 재확인 후 2차 매수 70% |
| **DOWNTREND** | MA20 < MA60 < MA120 AND ADX > adx_threshold **OR** KOSPI < KOSPI_MA120 | 신호 탐색 없이 전량 현금 대기 |
| **TRANSITION** | 위 3가지 미해당 | 신규 진입 차단, 기존 포지션 유지 |

→ [[TA지표/추세/MA_이동평균]] · [[TA지표/추세강도/ADX_추세강도]] · [[TA지표/변동성/볼린저밴드]]

### 분할 매수/매도

| 단계 | 조건 | 목표 비중 | 우선순위 |
|------|------|---------|---------|
| 1차 매수 | UPTREND 전환 첫날 | 40% | — |
| 2차 매수 | UPTREND + 종가 > MA20 (60거래일 이내) | 70% | — |
| 횡보 매수 | SIDEWAYS + 볼린저밴드 하단 | 30% | — |
| 1차 익절 | UPTREND → TRANSITION 전환 | 40% 유지 | 낮음 |
| 2차 청산 | 데드크로스 (MA20 < MA60) | 10% 유지 | 중간 |
| 전량 청산 | DOWNTREND 진입 or 볼린저밴드 상단 | 0% | 높음 |
| **ATR stop** | **낙폭 > ATR × 2.0** | **0%** | **최우선** |

→ [[매매원칙/분할매수매도_원칙]]

### ATR Stop-Loss

```
발동 조건: 당일 낙폭 > -(전일 ATR / 전일 종가 × 2.0)
결과: 즉시 전량 청산 — 모든 앞선 신호를 덮어씀

atr_period     = 14  (고정)
atr_multiplier = 2.0 (고정 — ATR 값 자체가 매일 변하므로 최적화 불필요)

적용 국면: UPTREND · SIDEWAYS · TRANSITION (포지션 있는 국면)
스킵 국면: DOWNTREND (포지션 없음)
```

→ [[TA지표/변동성/ATR_평균진폭]]

### 모멘텀 비례 배분

균등 배분 대신 추세 강도에 비례해 자본을 배분한다.

```
최종 비중 = 신호 크기 × (종목 모멘텀 / 유효 종목 모멘텀 합)
```

| 국면 | 모멘텀 윈도우 | 이유 |
|------|-------------|------|
| UPTREND | 126일 | 추세가 길게 이어지므로 장기 신뢰도 높음 |
| TRANSITION | 63일 | 방향 불확실, 중기 중간값 사용 |
| SIDEWAYS | 21일 | 단기 등락 반복, 빠른 반응 필요 |
| DOWNTREND | 사용 안 함 | 전량 청산 국면 |

→ [[TA지표/모멘텀/모멘텀]]

### 현금 운용 — 단기채 ETF 자동 주차

```
ETF 비중 = 1 - 주식 투자 비중 합계

예시:
  DOWNTREND        → 주식 0%  → ETF 100%
  UPTREND 1차 진입 → 주식 40% → ETF 60%
  UPTREND 2차 진입 → 주식 70% → ETF 30%
  ATR stop-loss    → 주식 0%  → ETF 100%

소액 매매 차단: ETF 비중 변화 < 1% → NaN(유지) 처리
실전 ETF: KODEX 단기채권 (153130.KS)
```

### Walk-Forward 최적화

```
IS 12개월 학습 → OOS 3개월 적용 (연 4회 갱신)
평가 기준: Calmar Ratio
탐색 파라미터:
  adx_threshold [15·20·25·30]
  adx_sideways  [10·15·20]
  → 12가지 조합

과적합 진단: OOS/IS Calmar 비율 ≥ 0.5 이상이어야 정상
```

→ [[최적화/Walk_Forward_최적화]] · [[최적화/Grid_Search_최적화]]
