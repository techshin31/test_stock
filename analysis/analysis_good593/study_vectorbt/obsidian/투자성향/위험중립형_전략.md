...
분기 실적 발표 직후(3·6·9·12월) FA 7지표를 재스크리닝해 보유 종목과 편입 후보를 검토한다.
Walk-Forward 최적화(3개월 주기)와 시점을 통일해 관리 포인트를 단일화한다.

> **일상 매매(1~4단계)와의 관계:**
> 분기 종목 교체는 일상 매매보다 상위 레이어다.
> 교체 결정 후 편출·편입 설정을 완료한 뒤 일상 매매(① 국면 판별 → ② 신호 생성 → ③ 자금 배분 → 주문 실행)가 정상 동작한다.

### 전체 흐름

```mermaid
flowchart TD
    A[분기 실적 발표 직후\n3·6·9·12월] --> B[FA 7지표 재스크리닝\n전 보유 종목 + 편입 후보]
    ...
    C -->|기존 종목\nFA 기준 미달| OUT[편출 확정\n매도 전용 모드 설정]
    OUT --> OUT2[새 매수 신호 차단\n기존 청산 로직만 동작\nATR stop · DOWNTREND · 데드크로스]
    OUT2 --> DEADLINE{20거래일 이내\n청산 완료?}
    DEADLINE -->|Yes — 신호로 청산| REMOVE[포트폴리오 제거\nWF 최적화 대상에서 제외]
    DEADLINE -->|No — 기한 도달| FORCE[시장가 강제 청산\n포트폴리오 제거]

    C -->|신규 종목\nFA 기준 충족| IN[과거 IS 데이터로\nWF 최적화 소급 실행]
    IN --> IN2[best_params\nuse_adx_mode 확정]
    IN2 --> IN3[다음 매매일부터\n정식 시그널 생성]

    KEEP --> WF[기존 종목 WF 최적화 실행\n분기 시작 시점 일괄 갱신]
```

### 혼재 기간 자금 배분

편출 종목의 청산 기한(최대 20거래일) 동안 편출 예정 종목과 신규 편입 종목이 공존한다.
**방법: 편출 종목을 매도 전용 모드(sell-only)로 설정해 자금 충돌을 방지한다.**

```
편출 예정 종목: make_signals()에서 매수 신호 생성 차단
               기존 청산 조건(ATR stop · DOWNTREND · 데드크로스)은 그대로 동작

신규 편입 종목: 정상 시그널 생성

→ build_size_df()는 편출 종목의 신규 자금 점유 없음
→ 편출 종목 청산 시 확보된 현금은 ETF로 자동 주차
  다음 매매 사이클에서 신규 종목 또는 ETF로 재배분
```

| 상태 | 매수 신호 | 매도 신호 | 자금 배분 |
|------|---------|---------|---------|
| 편출 예정 종목 | **차단** | 정상 동작 | 신규 점유 없음 |
| 신규 편입 종목 | 정상 동작 | 정상 동작 | 정상 배분 |
| 기존 유지 종목 | 정상 동작 | 정상 동작 | 정상 배분 |

### Cold Start 예외

기상장 종목(이미 시장에 상장된 회사)은 과거 종가 데이터로 WF 최적화를 소급 실행할 수 있어 Cold Start 문제가 없다.
Cold Start는 **IPO 신규 상장 종목**에만 해당하며, FA 7지표 스크리닝 특성상 재무 히스토리가 짧은 IPO 종목은 편입 후보에서 자연스럽게 걸러진다.

### 구현 포인트

```
make_signals() 호출 시 편출 예정 종목 리스트를 인자로 전달
  → 해당 종목은 매수 신호 생성 로직을 스킵
  → 기존 청산 조건(ATR stop, DOWNTREND, 데드크로스)은 그대로 동작

WF 최적화는 기존 종목과 신규 종목 모두 분기 시작 시점에 일괄 실행
  → 편출 예정 종목은 최적화 대상에서 제외
```
