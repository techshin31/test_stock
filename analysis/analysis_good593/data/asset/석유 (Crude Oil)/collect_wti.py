"""
WTI 원유 선물(CL=F) 가격 데이터 수집 — 백테스팅용 년도별 CSV 생성

출력: wti_{year}.csv (Date, Open, High, Low, Close, Volume)
  - Date:  YYYY-MM-DD 오름차순
  - OHLC:  소수점 2자리 (단위: USD/barrel)
  - Volume: 정수 (CL=F 실거래량)

Usage:
    python "collect_wti.py"
...
def parse_args() -> argparse.Namespace:
    today = date.today()
    p = argparse.ArgumentParser(description="WTI 원유(CL=F) 가격 수집 — 년도별 CSV")
...
def main() -> None:
    ...
    print(f"[WTI CL=F] {args.start_year} ~ {args.end_year}")
