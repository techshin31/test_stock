"""
구리 선물(HG=F) 가격 데이터 수집 — 백테스팅용 년도별 CSV 생성

출력: copper_{year}.csv (Date, Open, High, Low, Close, Volume)
  - Date:  YYYY-MM-DD 오름차순
  - OHLC:  소수점 4자리 (단위: USD/lb)
  - Volume: 정수 (HG=F 실거래량)

Usage:
    python "collect_copper.py"                          # 기본: 2021 ~ 올해
...
def save_by_year(df: pd.DataFrame) -> None:
    for year, group in df.groupby(df["Date"].str[:4]):
        path = OUT_DIR / f"copper_{year}.csv"
...
def parse_args() -> argparse.Namespace:
    today = date.today()
    p = argparse.ArgumentParser(description="구리(HG=F) 가격 수집 — 년도별 CSV")
...
def main() -> None:
    ...
    print(f"[구리 HG=F] {args.start_year} ~ {args.end_year}")
